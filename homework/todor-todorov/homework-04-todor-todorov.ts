// npx ts-node homework/todor-todorov/homework-04-todor-todorov.ts
...
class Employee {
  ...
  age: number;
  position: string;
  public constructor(name: string, age: number, position: string) {
    this.name = name;
  }
...
const person = new Employee('Todor', 30, 'QA');
console.log(person.getName());
person.changeName('Krisi');
console.log(person.getName());
...
class CarRental {
  ...
  public setCostPerDay(costPerDayNew: number): void {
    this.costPerDay = costPerDayNew;
  }
  public rentCar(days: number): void {
    const total = days * this.costPerDay;
    console.log(`Renting a ${this.carType} for ${days} days will cost: ${total}`);
  }
}

const Sedan = new CarRental('BMW', 50);
const Hetchback = new CarRental('Mercedes', 100);
const SUV = new CarRental('Audi', 200);

Sedan.rentCar(5);
Hetchback.rentCar(3);
SUV.rentCar(10);

Sedan.setCostPerDay(100);
Sedan.rentCar(5);
...
class Furniture {
  public typeFurniture: string;

  constructor(typeFurniture: string = 'Teakwood') {
...
class Chair extends Furniture {
  ...
  constructor(numberOfLegs: number, newTypeFurniture?: string) {
    if (newTypeFurniture) {
      super(newTypeFurniture);
    } else {
      super();
    }
    this.numberOfLegs = numberOfLegs;
  }
  setTypeWood(newtypeFurniture: string): void {
    this.typeFurniture = newtypeFurniture;
  }
...
const newChair = new Chair(7, 'Lipa');
console.log(
  `The type of furniture is ${newChair.typeFurniture} and the number of legs is ${newChair.getNumberofLegs()}`,
);

const secondChair = new Chair(4);
secondChair.setTypeWood('Oreh');
console.log(
  `The type of furniture is ${secondChair.typeFurniture} and the number of legs is ${secondChair.getNumberofLegs()}`,
);
