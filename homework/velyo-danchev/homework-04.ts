...
const employee1 = new Employee('Danny', 37, 'Coach');
console.log(employee1.getName());
console.log(employee1.changeName('Armando'));
console.log(employee1.getName());
...
class CarRental {
    ...
    setCostPerDay(newCostPerDay: number): string {
        this.costPerDay = newCostPerDay
        return `The new cost per day for ${this.carType} is $${this.costPerDay}`

    };

    getCostPerDay(rentDays: number): string {
        let totalRentCost = rentDays * this.costPerDay
        if (rentDays <= 0){
            return 'Cannot rent a car for less than 1 day.'
        } else {
            return `Renting a ${this.carType} for ${rentDays} days will cost: $${totalRentCost}`
        }
    }

    rentCar(): void {
        console.log(`To rent: ${this.carType} will cost $${this.costPerDay} per day.`)
    }
}

const carOne = new CarRental('Hatchback', 50);
const carTwo = new CarRental('Sedan', 77.78);
const carThree  = new CarRental('SUV', 55.57);

carOne.rentCar();
console.log(carTwo.getCostPerDay(7));
console.log(carThree.setCostPerDay(22));
...
const guitarOne = new Guitar();
const pianoOne = new Piano();
...
const animalOne = new Mammal('Kokochin');
const animalTwo = new Carnivore('Colombian guy');
const animalThree = new Herbivore('Vasko');
animalOne.eat();
animalTwo.eat();
animalThree.eat();
...
async function receivedUser() {
    console.log(`Receiving user Data`)
    let dataResult = await fetchUser();
    console.log(`Received data: ${dataResult.name}`)
...
async function runSteps() {
    ...
    const resultStepOne = await stepOne();
    console.log(resultStepOne);

    const resultStepTwo = await stepTwo();
