...
const arr1: number[] = [35, 15, 21, 14];
//const arr2: number [] = arr1.
const arrAscending: number[] = arr1.sort((x, y) => x - y);
console.log(arrAscending);

const arrEx3: number[] = [3, 4, 7, 1, 21, 18, 6];
let arrSorted: number[] = [...arrEx3];
arrSorted = arrSorted.sort((x, y) => x - y);
console.log(arrEx3);
console.log(arrSorted);

//Exercise 11 from homework 2
const arrEx11: number[] = [2, 3, 8, 21, 101, 2, 3, 101.5, 7, 8, 101, 21];
let arrNoDupl = [...arrEx11];

//Approach 1
function remDupl(arr: number[]) {
...
console.log(remDupl(arrNoDupl));

//Approach 2
arrNoDupl = arrNoDupl
  .reduce((arr, a) => (arr.includes(a) ? arr : [...arr, a]), [])
  .sort((a, b) => a - b);

console.log(arrNoDupl);

//Threads simulation through timeout
