...
let firstNumber: number = 65;
let secondNumber: number = 87;
...
let isLoggedIn: boolean = true; // change to false to test the other case
...
let text1: string = 'Apple'; // try "Banana" to test

let startsWithA: boolean = text1.startsWith('A');
...
let bigNumber1: bigint = 12345678901234567890n;
let bigNumber2: bigint = 98765432109876543210n;

let sum: bigint = bigNumber1 + bigNumber2;
...
let qaName: string = 'Simona';
let numberOfFunctionalities: number = 5;
const timePerFunctionality: number = 2; // hours per functionality
...
let numbers3: number[] = [10, 20, 10, 30, 40];

let uniqueNumbers: number[] = [...new Set(numbers3)];
...
const onlyInArr1 = arr1.filter((n) => !arr2.includes(n));
const onlyInArr2 = arr2.filter((n) => !arr1.includes(n));
...
const union: number[] = [...new Set([...arr3, ...arr4])];
