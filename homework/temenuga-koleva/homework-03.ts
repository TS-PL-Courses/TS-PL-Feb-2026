...
//npx ts-node homework/temenuga-koleva/homework-03.ts
...
for (const phoneNumber of phoneNumbers) {
    const newPhoneNumber = phoneNumber.replace(/^0/, "+359");
    console.log(`Original: ${phoneNumber}, New: ${newPhoneNumber}`);
}  
...
console.log(`Name: ${name}, Hour: ${hour}, Money: ${money}, Is Sick: ${isSick}, Activity: ${activity}`);
...
for (const client of clients) {
    console.log(`Name: ${client.name}, Account Type: ${AccountType[client.accountType]}`);
}   
...
/**
 * Multiply two numbers
 * @param {string}  firstName- the person's First Name to be printed
 * @param {string}  lastName - the person's Last Name to be printed
 * @param {string} [middleName] - the person's Middle Name to be printed (optional)
 * @returns {void} Prints the person's full name in the console
 */ 
...
/**
 * Concatenate first name, last name, and age into a single string
 * @param {string} [firstName] - the person's First Name (optional) 
 * @param {string} [lastName] - the person's Last Name (optional)
 * @param {number} [age] - the person's Age (optional)
 * @returns {string} The concatenated string
 */

function concatenateNames(firstName?: string, lastName?: string, age?: number): string {
    if (firstName === undefined || lastName === undefined || age === undefined) {
        const defaultFirstName = firstName ?? "First";
        const defaultLastName = lastName ?? "Last";
        const defaultAge = age ?? 20;
        return `Missing one or all arguments - first name, last name, or age. Default values: ${defaultFirstName} ${defaultLastName}, Age: ${defaultAge}`;
    }
 ...
 * @param {string} programmingLanguage - the programming language to be printed
 * @param {string} compliment - the compliment to be printed
 * @returns {void} Prints the message in the console
 */
function printLoveMessage(programmingLanguage: string, compliment: string): void {
    console.log(`I love ${programmingLanguage} because ${compliment}`);
}

printLoveMessage("TypeScript", "it's statically typed");
printLoveMessage("Python", "it's easy to learn");   
...
function countUpperAndLowerCase(str: string): { uppercaseCount: number; lowercaseCount: number } {
...
console.log(`Number of uppercase characters: ${counts.uppercaseCount}`);
console.log(`Number of lowercase characters: ${counts.lowercaseCount}`);
