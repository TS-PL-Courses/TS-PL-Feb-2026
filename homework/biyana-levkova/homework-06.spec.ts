...
test('First Sccenario', { tag: '@hw' }, async ({ page }) => {
  ...
  await test.step('Autocomplete suggestion', async () => {
    await page.locator('#autocomplete').fill('Bulgaria');
    await page.locator('#ui-id-1').filter({ hasText: 'Bulgaria' }).click();
  ...
  await test.step('Hide/Show element', async () => {
  ...
  await test.step('Hover over element', async () => {
    ...
    await expect(page.locator('.mouse-hover-content a[href="#top"]')).toBeVisible();
  });
});
...
test('Second Scenario', { tag: '@hw2' }, async ({ page }) => {});
