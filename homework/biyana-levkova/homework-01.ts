// Homework 01
// npx ts-node "homework/biyana levkova/homework-01.ts"
...
let num1: number = 10;
let num2: number = 20; 
let num1Swapped = num2;
let num2Swapped = num1;

console.log(num1, num2);
console.log(num1Swapped, num2Swapped);
...
let tempCelcius: number = -2;
let tempFahrenheit: number = tempCelcius*9/5+32;
...
// Step 3: Log the result
let myString: string = 'loremIpsum';
let stringIsEmpty: boolean = myString.length<1;
...
// Step 3: Log the result
let myString1: string = 'Aloha';
let startsWithA: boolean = myString1.charAt(0).toLowerCase() === "a";
...
let typescriptLanguage: string = 'Typescript is fun';
console.log(typescriptLanguage.length);
console.log(typescriptLanguage.toUpperCase());
console.log(typescriptLanguage.toLowerCase());
let favoriteLanguage: string = typescriptLanguage.split(' ')[0];
...
// Step 3: Log the result.
let bigInt1: bigint = 30000000000n;
let bigInt2: bigint = 8000000n;
...
const resultArray: number[] = numArray.map(numArray => numArray*2);
...
studentsArray.push({name: 'Bob', grade: 85});

studentsArray.forEach(students=>{console.log(students.name)});
...
let chairId= printId(951367);
let tableId= printId('OakTable');
...
function QaHours(QaName: string, featuresCount: number) : void {
    let featureTimeNeeded: number = 2;
    let totalTimeNeeded: number = featuresCount*2;
    console.log(`The Qa ${QaName} will need ${totalTimeNeeded} hours to complete ${featuresCount} project/s.`);
}
let AleksandarQa= QaHours('Aleksandar', 3);
let BiyanaQa= QaHours('Biyana',2);
...
const user1: {id: number|string, name: string,} = {
    id: 'skjcUmd',
    name: 'Milena',
}
const user2: {id: number|string, name: string,} = {
    id: 856982,
    name: 'Strahil',
}
console.log(user1);
...
const firstArray: number[] = [10, 20, 10, 30, 40];
