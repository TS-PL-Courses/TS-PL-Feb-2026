// npx ts-node homework/daniela-metodieva/homework-01/exercise-1.ts
...
let startsWithA: boolean = stringA.startsWith('A');
if (startsWithA) {
  console.log(`The string "${stringA}" starts with the letter 'A'.`);
} else {
  console.log(`The string "${stringA}" does not start with the letter 'A'.`);
}
...
let bigInt1: bigint = 3498539475943593419858n;
let bigInt2: bigint = 1000000000000000000000n;
...
let qaName: string = 'Dani';
let numberOfFunctionalities: number = 4;
let timePerFunctionality: number = 2;
let totalTimeNeeded: number = numberOfFunctionalities * timePerFunctionality;
console.log(
...
let arrayWithDuplicates: number[] = [10, 20, 10, 30, 40, 10];
let uniqueArray: number[] = Array.from(new Set(arrayWithDuplicates));
...
let array2: number[] = [3, 4, 8, 9];
let difference: number[] = [
  ...array1.filter((num) => !array2.includes(num)), // Elements in array1 but not in array2
  ...array2.filter((num) => !array1.includes(num)), // Elements in array2 but not in array1
];
console.log(`Difference between the two arrays: ${difference}`);
...
let array4: number[] = [3, 4, 5];
let union: number[] = Array.from(new Set([...array3, ...array4]));
