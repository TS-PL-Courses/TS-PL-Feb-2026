...
//npx ts-node homework/katerina-karaboeva/typescript-homeworks/katerina-ts-homework-04.ts
...
const employee = new Employee('Katerina', 33, 'QA');
employee.getName();
console.log(employee.name);
employee.changeName('Ivelina');
console.log(employee.name);
...
class CarRental {
  ...
  rentCar(daysCount: number): void {
    console.log(
      `Renting a ${this.carType} for ${daysCount} days will cost: $${daysCount * this.costPerDay}`,
    );
  }
}

const sedan = new CarRental('Sedan', 150);
const suv = new CarRental('SUV', 250);
const hatchBack = new CarRental('Hatchback', 100);

sedan.rentCar(3);
suv.rentCar(10);
hatchBack.rentCar(12);

sedan.setCostPerDay(20);
sedan.rentCar(3);
...
//Solution:
class Furniture {
  furnitureType = 'Teakwood';

  setFurnitureType(newFurnitureType: string): void {
...
const chair = new Chair(4);
chair.setFurnitureType('Table');
console.log(`Chair 1: Type - ${chair.furnitureType}, Legs - ${chair.getChairLegs()}`);

const chair2 = new Chair(6);
chair2.setFurnitureType('Chair');
console.log(`Chair 2: Type - ${chair2.furnitureType}, Legs - ${chair2.getChairLegs()}`);
...
class Guitar extends Instrument {
  public playMusic(): string {
    return 'Playing Guitar music';
  }
}

class Piano extends Instrument {
  public playMusic(): string {
    return 'Playing Piano music';
  }
}
...
class Carnivore extends Mammal {
  public override eat(): void {
    console.log(`${this.name1} is a carnivore and eats meat`);
  }
}

class Herbivore extends Mammal {
  public override eat(): void {
    console.log(`${this.name1} is a herbivore and eats plants`);
  }
}
