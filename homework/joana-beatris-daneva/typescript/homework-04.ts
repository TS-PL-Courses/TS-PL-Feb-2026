...
const employee = new Employee('Joana');

console.log('Original name is: ', employee.getName());

employee.changeName('Irena');

console.log('Updated name is: ', employee.getName());
...
console.log(sedan.rentCar(2));
console.log(suv.rentCar(5));
...
class Furniture {
  public woodType: string = 'Teakwood';
}
...
const dog: Carnivore = new Carnivore('Bob');
const rabbit: Herbivore = new Herbivore('Peter');
...
type MyUser = { id: number; name: string };

function fetchUser(): Promise<MyUser> {
  return new Promise<MyUser>((resolve) => {
    const user: MyUser = { id: 1, name: 'Alice' };
