...
const second: number = 9;
const third: number = 4;
...
const moshtenExtension: string = 'Moshten Extension';
if (moshtenExtension.length > 10) {
...
const arr1 = ['moshten', 'extension', 'baby'];
let upperCaseArr = arr1.map((string) => string.toUpperCase());
...
// Step 3: Store the result in a new variable.
// Step 4: Log the result.
const arr2 = ['moshten', 'extension', 'baby', 'dostaMoshtenNaistina'];
let longerThan5 = arr2.filter((string) => string.length > 5);
...
let result = arr3.reduce((max, current) => (current > max ? current : max), arr3[0]);
console.log(result);
...
const items: Item[] = [
  { name: 'Moshten Extension1', price: 5, quantity: 5 },
  { name: 'Moshten Extension2', price: 6, quantity: 3 },
  { name: 'Moshten Extension3', price: 7, quantity: 2 },
];
...
const uniqueValues: number[] = variables.reduce((unique, current) => {
