...
class CarRental {
    ...
    rentCar(days:number):void{
        ...
        return console.log(`Renting a ${this.carType} for ${days} days will cost ${totalCost}`)
    }
}
...
console.log("\nExercise 3 - Car Rental Service (Encapsulation)");
hatchbackRental.rentCar(3);
sedanRental.rentCar(5);
suvRental.rentCar(2);
hatchbackRental.setCostPerDay(60);
console.log(`New cost for Hatchback: $${hatchbackRental.getCostPerDay()}`);
hatchbackRental.rentCar(3);
...
const chair2 = new Chair("Oakwood", 3);
console.log("\nExercise 4 - Furniture Factory (Inheritance)");
...
class Guitara extends Instrument{
    playMusic():void{
...
function waitTwoSeconds():Promise<string>{
    return new Promise((resolve) => {
        setTimeout(() => {
            resolve("Done waiting! for exercise 7");
...
async function runTask():Promise<void>{
    ...
    console.log(`Result: wait from exercise 7 ${result} done for exercise 8`);
...
function fetchWithError():Promise<string>{
    return new Promise((resolve,reject) =>{
        setTimeout(() => {
            ...
            if(isSuccess){
                resolve("Success! for exercise 10");
            }else{
                reject("Something went wrong for exercise 10");
            }
        }, 1000);
    });
}
