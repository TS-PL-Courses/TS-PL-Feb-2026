// npx ts-node homework/mihaelina-lilova/Typescript/homework-04.ts
...
const newEmpolyee = new Employee('John');

console.log(newEmpolyee.getName());
newEmpolyee.changeName('Joan');
console.log(newEmpolyee.getName());
...
const carTwo = new CarRental('Sedan', 80);
const carThree = new CarRental('SUV', 100);

carOne.rentCar(5);
carTwo.rentCar(10);
carThree.rentCar(20);

carOne.setCostPerDay(40);
carOne.rentCar(5);
...
class Furniture {
  ...
  //   public constructor(public type: string = 'Teakwood') {
  //     this.type = type;
  //   }
}
...
class Chair extends Furniture {
  ...
  changeType(newType: string): string {
...
const chair2 = new Chair(2, 6);
chair2.changeType('Wood');
chair2.chairInfo();
...
class Guitar extends Instrument {
  ...
  public playMusic(): void {
    console.log(`${this.name} music is played on a guitar`);
  }
}
...
class Piano extends Instrument {
  ...
  public playMusic(): void {
    console.log(`${this.name} music is played on a piano`);
  }
}

const newInstrument1 = new Guitar('Classic guitar');
const newInstrument2 = new Piano('Royal');

newInstrument1.playMusic();
newInstrument2.playMusic();
...
function fetchUser(): Promise<UserOne> {
  return new Promise((resolve) => {
    setTimeout(() => {
...
async function fetchCaller() {
  try {
    const callerResult = await fetchWithError();
    console.log(callerResult);
  } catch (err) {
