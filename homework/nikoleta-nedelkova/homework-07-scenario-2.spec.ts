...
[
  ...
  test(
    `Verify user creation and deletion: ${scenario}`,
    {
      tag: ['@ui', '@clients'],
      annotation: [
    ...
    async ({ sharedSteps, landingSteps, clientsSteps, addClientSteps }) => {
      ...
      const clientName = await addClientSteps.fillClientName();
     

      await addClientSteps.clickSaveButton();
      await addClientSteps.navigateToClientsList();
      await addClientSteps.deleteClient(clientName);
    },
  );
});
